...
def fusion(img1, img2, name_path):
    # Params
    ...
    outImage = np.multiply(np.divide(outImage - np.min(outImage),(np.max(outImage) - np.min(outImage))),65535)
    outImage = outImage.astype(np.uint16)
